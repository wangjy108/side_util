...
class Img4process():
    def __init__(self, fig, N_in_line, N_in_column, if_header, **args):
        self.fig = fig
...
if __name__ == "__main__":
    ...
    parser.add_argument('--InputFig', type=str, required=True, 
                        help='input patent fig')
    ...
    Img4process(args.InputFig, args.N_in_line, args.N_in_column, args.if_header).run()
    
    #print (args.InputFig.split(".")[0])
    logging.info(f"Processing {args.InputFig}")
    try:
        os.path.exists(args.InputFig.split(".")[0])
    except Exception as e:
        logging.info(f"Failed to process {args.InputFig}")
    else:
        os.chdir(args.InputFig.split(".")[0])
        _path = [cc for cc in os.listdir(".")]
        if not _path:
            logging.info(f"Faied with {args.InputFig}, check and run again")
            os.chdir(main_dir)
            os.system(f"rm -f {args.InputFig.split('.')[0]}")
        else:
            if args.parallel:
